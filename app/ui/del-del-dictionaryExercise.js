...
/* -------------------------------------------------------------
   Language‑agnostic column builder (dictionary‑specific)
   ------------------------------------------------------------- */
function buildTokenColumn(translations, langs) {
    ...
    // -----------------------------------------------------------------
    // 1️⃣ Create a <span> for every translation and register it for the
    //    speech controller (global tokenEls / transEls arrays live in the
    //    surrounding scope of this file).
    // -----------------------------------------------------------------
    translations.forEach((txt, idx) => {
        ...
        // First language (idx===0) is visible by default.
        if (idx === 0) col.appendChild(span);

        // Keep a reference for the speech controller.
        const tokenIdx = tokenEls.length; // one token per column
        if (!transEls[tokenIdx]) transEls[tokenIdx] = [];
        transEls[tokenIdx].push(span);
    });

    // -----------------------------------------------------------------
    // 2️⃣ Store the column itself in tokenEls – the controller uses the
    //    index of the column to know which token we’re on.
    // -----------------------------------------------------------------
    tokenEls.push(col);
    ...
    // Delegated click listener (covers spans added later when Display
    // check‑boxes are toggled).
...
export async function renderDictionaryExercise(mainEl, exerciseMeta, uiLang) {
 ...
 //               const col = buildTokenColumn(srcWord, translations, columnLangs);
                ...
                const col = buildTokenColumn(translations, columnLangs);
