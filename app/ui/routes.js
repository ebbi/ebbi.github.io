...
import { ensureExercisesLoaded } from './menu.js';
...
export async function testExerciseHandler({ lang, id } = {}) {
    ...
    // 1️⃣ Pull data from the "Suitcase" (Session Storage)
    const customDataString = sessionStorage.getItem('custom_quiz_data');
    let customData = null;
    let sectionTitle = "";

    if (customDataString) {
        const payload = JSON.parse(customDataString);
        // We handle both old format (array) and new format (object)
        if (payload.rows) {
            customData = payload.rows;
            sectionTitle = payload.title;
        } else {
            customData = payload;
        }
        // Clear it so it doesn't accidentally trigger on a different lesson later
        sessionStorage.removeItem('custom_quiz_data');
    }

    // 2️⃣ Load Lesson Metadata
    const { EXERCISES } = await import("../data/exercises.js");

    if (EXERCISES.length === 0) {
        // CHANGE: Import ensureExercisesLoaded instead of loadExercises
        const { ensureExercisesLoaded } = await import("./menu.js");
        await ensureExercisesLoaded();
    }

    const meta = EXERCISES.find(e => e.id === id);

    // 3️⃣ Render the Header
    const mainEl = await renderHeader(lang);
    const titleEl = mainEl.querySelector('h1') || mainEl.querySelector('.title');

    // 4️⃣ Update Title (Mobile-Friendly Layout)
    if (titleEl && meta) {
        const lessonTitle = meta.title[lang] || meta.title['en'] || id;

        if (sectionTitle) {
            // Mobile-first: Put section on a new line if it's long, 
            // and use a slightly smaller font size for the subtitle.
            titleEl.innerHTML = `${lessonTitle}<br><small style="font-size: 0.7em; opacity: 0.8;">Section: ${sectionTitle}</small>`;
        } else {
    ...
    await initMultipleChoicePage(lang, id, customData);
}
