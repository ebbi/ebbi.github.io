    ...
    const State = {
        data: {
            ...
            media: {
                ...
                voiceSpeedPreset:
                    localStorage.getItem("localStorageVoiceSpeedPreset") ||
                    "normal",
                showWordBreakdown:
            ...
            // NEW: Track opened documents for progress
            openedDocuments: (() => {
                try {
                    return (
                        JSON.parse(localStorage.getItem("openedDocuments")) ||
                        []
                    );
                } catch {
                    return [];
        ...
        defaults: {
            ...
            voiceSpeedPresets: {
                ...
                slow: { speed: 0.7, delay: 1.5, pitch: 1.0, label: "Slow" },
                slower: {
                    speed: 0.5,
                    delay: 2,
                    pitch: 0.95,
        ...
        save(key) {
            ...
            if (key === "media") {
                ...
                localStorage.setItem(
                    "localStorageVoiceSpeedPreset",
                    this.data.media.voiceSpeedPreset,
                );
                localStorage.setItem(
            ...
            if (key === "openedDocuments") {
                localStorage.setItem(
                    "openedDocuments",
                    JSON.stringify(this.data.openedDocuments),
                );
            }
            if (key === "srs") {
...
<div class="voice-speed-section">
    <label class="voice-speed-title">
        <span class="material-icons" style="vertical-align:middle;margin-right:6px;">speed</span>
        ${t("voice_speed", "Voice Speed")}
    </label>
    <div class="voice-speed-options">
        ${["normal", "slow", "slower"]
            .map((preset) => {
                const p = State.defaults.voiceSpeedPresets[preset];
                const isActive = media.voiceSpeedPreset === preset;
                return `
            <label class="voice-speed-option ${isActive ? "active" : ""}">
                <input type="radio" name="voiceSpeed" value="${preset}"
                       ${isActive ? "checked" : ""}
                       onchange="App.applyVoiceSpeedPreset('${preset}')">
                <span class="voice-speed-radio-dot"></span>
                <div class="voice-speed-info">
                    <span class="voice-speed-label">${t("speed_" + preset, p.label)}</span>
                    <span class="voice-speed-detail">${p.speed}x · ${p.delay}s delay</span>
                </div>
                <button class="voice-speed-test-btn"
                        onclick="event.stopPropagation(); App.testVoiceSpeed('${preset}')"
                        title="${t("test_speed", "Test this speed")}">
                    <span class="material-icons">volume_up</span>
                </button>
            </label>`;
            })
...
<div class="voice-speed-section">
    ...
    <div class="voice-speed-options">
        ${["normal", "slow", "slower"]
            ...
            <label class="voice-speed-option ${isActive ? "active" : ""}">
                ...
                <div class="voice-speed-info">
                    <span class="voice-speed-label">${t("speed_" + preset, p.label)}</span>
                    <span class="voice-speed-detail">${p.speed}x</span>
                </div>
                <button class="voice-speed-test-btn"
                        onclick="event.stopPropagation(); App.testVoiceSpeed('${preset}')"
                        title="${t("test_speed", "Test this speed")}">
        ...
        Progress: {
            render() {
                ...
                const lang = State.data.lang;
                const t = Services.I18n.t;
                const opened = State.data.openedDocuments;

                let html = `<div class="progress-page-container">`;
                html += `<h2 class="progress-page-title">${t("learning_progress", "Learning Progress")}</h2>`;

                manifest.learningObjects.forEach((category) => {
                    const catTitle = category.title[lang] || category.title.en;
                    const docs = category.documents || [];
                    const total = docs.length;
                    const completed = docs.filter((doc) =>
                        opened.includes(doc.id),
                    ).length;
                    const percent =
                    ...
                    html += `
                     <div class="progress-category-card">
                         <div class="progress-cat-header">
                             <span class="progress-cat-title">${UI.escapeHtml(catTitle)}</span>
                             <span class="progress-cat-stats">${completed} / ${total}</span>
                         </div>
                         ...
                         ${
                             nextDoc
                                 ? `
                             <button class="btn-next-exercise" data-action="navigate" data-route="doc/${nextDoc.id}">
                                 <span class="material-icons">arrow_forward</span>
                                 <span class="btn-next-text">${t("next_exercise", "Next")}: ${UI.escapeHtml(nextDoc.title[lang] || nextDoc.title.en)}</span>
                             </button>
                         `
                                 : `
                             <div class="progress-completed-badge">
                                 <span class="material-icons">check_circle</span>
                                 <span>${t("category_completed", "Completed!")}</span>
                             </div>
                         `
                         }
                     </div>
      ...
      <a class="toolbar__title-link" href="../../" id="toolbar-title-link">${Services.I18n.t("zabon", "Zabon")}</a>
                ...
                <button class="material-icons toolbar-btn"
                        data-action="navigate" data-route="blogs">article</button>
                                     <button class="material-icons toolbar-btn"
                     data-action="navigate" data-route="progress">leaderboard</button>
            </nav>
    ...
    `;
        ...
        handleDocumentClick(documentId) {
            ...
            if (!State.data.openedDocuments.includes(documentId)) {
                State.data.openedDocuments.push(documentId);
                State.save("openedDocuments");
            }
     ...
     <div class="media-row">
        ...
        resetSettingsToDefaults() {
            ...
            State.data.media.showWordBreakdown =
                State.defaults.media.showWordBreakdown;
            State.data.media.voiceSpeedPreset =
                State.defaults.media.voiceSpeedPreset;
            ...
            localStorage.removeItem("localStorageShowWordBreakdown");
            localStorage.removeItem("localStorageVoiceSpeedPreset");
    ...
    `;
        ...
        playSequence() {
            ...
            const playNext = () => {
                ...
                // Define the element before checking it
            ...
            // Inside App.playSequence(), replace the existing speakCurrent function:
            const speakCurrent = () => {
                ...
                // --- 1. Cross-Browser Word Highlighting Setup ---
                let sentenceWords = null; // For Thai source
                let sentenceUid = null;
                let translationWordSpans = []; // For En/Fa translations
                ...
                if (element.hasAttribute("data-uid")) {
                    ...
                    if (element.classList.contains("source")) {
                        sentenceUid = uid;
                        const sData = App.findSentenceByUid(sentenceUid);
                        if (sData && sData.words && sData.words.length > 0) {
                            sentenceWords = sData.words;
                        }
                    } else if (element.classList.contains("trans")) {
                        isTranslation = true;
                        translationWordSpans = Array.from(
                            element.querySelectorAll("span.word-span"),
                        );
                    }
                }

                const totalWords = sentenceWords
                    ? sentenceWords.length
                    : translationWordSpans.length;
                let fallbackTimer = null;
                let fallbackIndex = 0;

                // ------------------------------------------------------------------
                // ISOLATED HIGHLIGHTING LOGIC
                // Thai source + word breakdown path is UNCHANGED.
                // Translation path is handled separately.
                // ------------------------------------------------------------------
                const applyWordHighlight = (index) => {
                    // Clear previous word-level highlights only
                    document
                        .querySelectorAll(
                            ".word-span.active-highlight, .sent-word-item.active-highlight",
                        )
                        .forEach((el) => {
                            el.classList.remove("active-highlight");
                        });
                    ...
                    if (isTranslation) {
                        ...
                        if (index >= 0 && index < totalWords) {
                            const span = translationWordSpans[index];
                            if (span) span.classList.add("active-highlight");
                        }
                    } else {
                        // ── THAI SOURCE PATH ──────────────────────────────────────
                        // Do NOT highlight the parent .stack-item.source element to avoid
                        // painting the entire sentence yellow. Highlight individual words.
                        if (index >= 0 && index < totalWords) {
                            if (sentenceWords) {
                                // Highlight the matched word in the Thai source
                                const sourceWordId = `source-${sentenceUid}-w-${index}`;
                                const sourceEl =
                                    document.getElementById(sourceWordId);
                                if (sourceEl)
                                    sourceEl.classList.add("active-highlight");

                                // If "Show Word Breakdown" is on, also highlight the
                                // corresponding breakdown card
                                const breakdownId = `${sentenceUid}-card-${index}`;
                                const breakdownEl =
                                    document.getElementById(breakdownId);
                                if (breakdownEl)
                                    breakdownEl.classList.add(
                                        "active-highlight",
                                    );
                            } else {
                                // Fallback: if no word data is available, highlight the whole element
                ...
                // --- 2. Clean Repeat & Progression Logic ---
                let currentRepeat = 0;
                const speakNextRepeat = () => {
                    ...
                    Services.MediaService.speak(
                        ...
                        () => {
                            ...
                            // Start Time-Based Fallback Timer
                            if (totalWords > 0 && !fallbackTimer) {
                                fallbackIndex = 0;
                                applyWordHighlight(0);
                                const baseMsPerWord = 400;
                                const interval =
                                    baseMsPerWord / State.data.media.speed;
                                fallbackTimer = setInterval(() => {
                                    fallbackIndex++;
                                    if (fallbackIndex >= totalWords) {
                                        clearInterval(fallbackTimer);
                                        fallbackTimer = null;
                                    } else {
                                        applyWordHighlight(fallbackIndex);
                                    }
                                }, interval);
                            } else if (totalWords === 0 && !isTranslation) {
                                // Fallback: highlight the whole Thai sentence if no word data exists
                                element.classList.add("active-highlight");
                            }
                        },
                        () => {
        ...
        applyVoiceSpeedPreset(presetName) {
            const presets = State.defaults.voiceSpeedPresets;
            const preset = presets[presetName];
            if (!preset) return;

            // Apply all three parameters at once
            State.data.media.speed = preset.speed;
            State.data.media.delay = preset.delay;
            State.data.media.pitch = preset.pitch;
            State.data.media.voiceSpeedPreset = presetName;
            State.save("media");

            // Update the visual active state on radio labels
            document.querySelectorAll(".voice-speed-option").forEach((opt) => {
                opt.classList.toggle(
                    "active",
                    opt.querySelector("input").value === presetName,
                );
            });

            // Refresh the media bar to reflect new values
            const mediaBar = document.getElementById("media-player-container");
            ...
            this.showNotification(
                Services.I18n.t("voice_speed_set", "Voice speed: ") +
                    State.defaults.voiceSpeedPresets[presetName].label,
            );
        },
        testVoiceSpeed(presetName) {
            const preset = State.defaults.voiceSpeedPresets[presetName];
            if (!preset) return;

            // Temporarily apply the preset's parameters for the test utterance
            const prevSpeed = State.data.media.speed;
            const prevPitch = State.data.media.pitch;

            State.data.media.speed = preset.speed;
            State.data.media.pitch = preset.pitch;

            // A natural Thai test sentence with varied tones and lengths
            const testSentence = "สวัสดีครับ ยินดีต้อนรับสู่แอปเรียนภาษาไทย";

            const utterance = new SpeechSynthesisUtterance(testSentence);
            utterance.lang = Services.MediaService.getLocaleFor("th");
            utterance.rate = preset.speed;
            utterance.pitch = preset.pitch;

            // Use the currently selected voice
            if (State.data.media.voice) {
                const voice = Services.MediaService.cachedVoices.find(
                    (v) => v.name === State.data.media.voice,
                );
            ...
            // Restore previous values after speech ends
            utterance.onend = () => {
                State.data.media.speed = prevSpeed;
                State.data.media.pitch = prevPitch;
            };
...
*/
        ...
        toggleMenu(type) {
            ...
            `;
                ...
                options = {
                    "font-modern": t("font_modern"),
                    "font-traditional": t("font_traditional"),
                };
